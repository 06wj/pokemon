...
const compatibleVersion = '2.0.0-alpha.5';
...
export function getToonGeometryUnsupportedReason(mesh: Hilo3d.Mesh): string | null {
  ...
  if (mesh.useInstanced || mesh.instanceCount > 1) {
    // alpha.5 does not inject its built-in HILO_INSTANCED variant into custom GLSL.
...
export class ToonGeometryMaterial extends Hilo3d.MaterialInstance {
  // alpha.5's draw validation uses this public custom-GLSL marker for arbitrary MRT layouts.
