...
export const LIVING_TUNING = {
  ...
  repeatElectricSeconds: 54, electricQuietRadius: 2.4,
  elementalSignalSeconds: 2.2, elementalReactionSeconds: 10,
  elementalRepliesPerSignal: 2, maxElementalReplies: 3,
  elementalRadius: { fire: 4.3, water: 4, electric: 4.2 },
  recentEventSeconds: 40, maxRecentEvents: 96,
} as const;
...
export interface LivingNature {
  ...
  abilities: { ignite: boolean; waterFlowers: boolean; smellFlowers: boolean; electric: boolean };
  home: EcologyPoint;
}
...
const authored: Record<string, Partial<Omit<LivingNature, 'interests' | 'abilities'>> & {
  ...
  '025': { home: { x: -8.2, z: 3.3 }, curiosity: 1, playfulness: .9, interests: { flowers: 1, warmth: .85 }, abilities: { smellFlowers: true, electric: true } },
...
export function livingNature(id: string, types: readonly string[]): LivingNature {
  ...
  return {
    ...
    abilities: { ignite: false, waterFlowers: false, smellFlowers: false, electric: false, ...row?.abilities },
  };
}
